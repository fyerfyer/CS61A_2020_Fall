...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        atk=other_card.attack
        defe=other_card.defense
        other_card.attack=defe
        other_card.defense=atk
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(orig_opponent_deck_length):
            if copy[i].attack==atk or copy[i].defense==dfs:
                del_oppo.append(i)
        opponent.deck.cards=[copy[i] for i in del_oppo]
