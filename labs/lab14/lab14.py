...
def num_splits(s, d):
    ...
    def helper(index,sum1,sum2):
        ...
        if index==len(s):
            ...
            helper(index+1,sum1+s[index],sum2)
            helper(index+1,sum1,sum2+s[index])
    helper(0,0,0)
    return count//2
...
class CheckingAccount(Account):
    ...
    def deposit_check(self,check):
        if self.holder!=check.payable_to:
            print('The police have been notified.')
        elif check.deposited is True:
            print('The police have been notified.')
        else:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edit_char,cost_tot=helper_align(skeleton_idx+1,code_idx+1)
            possibilities.append((skel_char+edit_char,cost_tot))
        # Insert
        edit_char,cost_tot=helper_align(skeleton_idx,code_idx+1)
        edit_char="+["+code[code_idx]+"]"+edit_char
        possibilities.append((edit_char,cost_tot+1))
        # Delete
        edit_char,cost_tot=helper_align(skeleton_idx+1,code_idx)
        edit_char="-["+skeleton[skeleton_idx]+"]"+edit_char
        possibilities.append((edit_char,cost_tot+1))
...
def foldl2(link, fn, z):
    ...
    def step(x, g):
        "*** YOUR CODE HERE ***"
    #x is the list.first part,and g is the solved part's solution
    #the usage of the function is to merge x and solved z together
