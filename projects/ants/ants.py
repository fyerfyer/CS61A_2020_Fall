...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while True:
            ...
            if bee is not None and self.min_range<=distance<=self.max_range:
                return bee
            distance+=1
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting==0:
            ...
            if HungryAnt.eat_bee(self,bee) is True:
...
# END Problem EC
        ...
        if self.is_queen==True:
            ...
            while True:
                if cur_place is None:
                    break
                elif cur_place.ant is not None and cur_place.ant.buffed==False:
    ...
    def reduce_armor(self, amount):
        ...
        if self.is_queen is True and self.armor<=0:
            bees_win()
        # END Problem EC
...
def apply_status(status, bee, length):
    ...
    def create_action(gamestate):
        nonlocal length
        if length>0:
            new_action(gamestate)
            length-=1
        else:
            if bee.direction is False:
                bee.direction=True
            old_action(gamestate)
    bee.action=create_action
    # END Problem Optional 4
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not target.has_scaried:
            apply_status(make_scare,target,2)
            target.has_scaried=True
        # END Problem Optional 4
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place is not beehive:
            if cur_place.ant is not None and cur_place.ant is not self:
                dic[cur_place.ant]=dis
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        ret = 2 - 0.2 * distance - 0.05 * self.insects_shot
        return ret
        # END Problem Optional 5
