...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator,MacroProcedure):
            return scheme_eval(operator.apply_macro(rest,env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            ...
            Frame.define(child_frame,symbol,vals.first)
            vals=vals.rest
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def convert_list(arg):
            ...
            if arg is not nil:
                python_args.append(arg.first)
                return convert_list(arg.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_numberp(expressions.rest.first):
            bind=expressions.rest.first
        else:
            bind=scheme_eval(expressions.rest.first,env)
        env.bindings[target]=bind
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambda_form=do_lambda_form(Pair(target.rest,expressions.rest),env)
        env.bindings[target.first]=lambda_form
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    cur_bind=bindings
    while cur_bind is not nil:
        bind=cur_bind.first
        validate_form(bind,2,2)
        names=Pair(bind.first,names)
        val=eval_all(bind.rest,env)
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    validate_form(expressions,2)
    func=expressions.first
    if isinstance(func,Pair) and scheme_symbolp(func):
        macro_name=func.first
        args=func.rest
        ...
        mistake=func.first if isinstance(func,Pair) else func
        raise SchemeError('non-symbol: {0}'.format(mistake))
    # END Problem 20
